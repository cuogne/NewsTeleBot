...
## Lưu ý khi phát triển thêm feed mới:

Nếu bạn muốn phát triển thêm nguồn tin mới theo cấu trúc hiện có, vui lòng đảm bảo các bước sau.

### B1. Thêm feed trong [config/resource.go](./config/resource.go)

Tạo feed mới với cấu trúc của type `Resource`, rồi thêm vào slice `Feeds`:
...
### B2. Tạo table mới trong `db/database.sql`

Tạo table mới theo đúng schema các feed table hiện có:
...
Ví dụ nếu `Category` là `tinmoi`, table cũng phải tên là `tinmoi`:
...
### B3. Giữ `Category` trùng tên table

Tên `Category` trong [config/resource.go](config/resource.go) phải trùng với tên table trong [db/database.sql](db/database.sql). Code repository đang dùng `Category` trực tiếp làm tên table khi query database, nên nếu không khớp thì bot sẽ không select/insert được dữ liệu.

Nên đặt `Category` dạng lowercase, không dấu, không khoảng trắng, ví dụ:

- `tinmoi`
- `lichthipkt`
- `tintuchcmus`

Mỗi category hiện chỉ giữ lại 20 bài mới nhất trong database.

Nếu bạn không muốn dùng rule `table name = category`, bạn sẽ phải sửa lại code trong `internal/repository` hoặc `internal/service`.

### B4. Kiểm tra `Format`

Hiện crawler hỗ trợ sẵn các format trong [internal/crawler/crawl.go](internal/crawler/crawl.go):

- `rss`: dùng cho RSS feed.
- `api`: dùng cho JSON API kiểu WordPress REST, có field `title.rendered` và `link`.
- `html`: dùng crawler HTML hiện tại, selector đang viết riêng cho cấu trúc trang HCMUS.

Nếu feed mới dùng được một trong các format trên thì không cần viết crawler mới. Nếu website/API có cấu trúc khác, hãy thêm crawler mới trong thư mục [internal/crawler](internal/crawler), rồi thêm case tương ứng trong [internal/crawler/crawl.go](internal/crawler/crawl.go):

```go
// crawl.go
// ...
case "your_format":
	listArticles, err := yourCrawlFunction(feed.URL, feed.Category)
